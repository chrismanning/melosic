...
struct KCategorizedView::Private::Block
{
    ...
    static bool lessThanPtr(const QSharedPointer<Block> left, const QSharedPointer<Block> right)
    {
        Q_ASSERT(left && left->firstIndex.isValid());
        Q_ASSERT(right && right->firstIndex.isValid());
        return left->firstIndex.row() < right->firstIndex.row();
    }
...
QStyleOptionViewItemV4 KCategorizedView::Private::blockRect(const QModelIndex &representative)
{
    ...
    auto block = blockForIndex(representative);
    QPoint pos = blockPosition(block);
    pos.ry() -= height;
    option.rect.setTopLeft(pos);
    option.rect.setWidth(viewportWidth() + categoryDrawer->leftMargin() + categoryDrawer->rightMargin());
    option.rect.setHeight(height + blockHeight(block));
...
QPoint KCategorizedView::Private::blockPosition(QSharedPointer<Block> block)
{
...
int KCategorizedView::Private::blockHeight(QSharedPointer<Block> block)
{
    if (block->collapsed) {
        return 0;
    }
...
{
    for (QHash<QString, QSharedPointer<Block>>::Iterator it = blocks.begin(); it != blocks.end(); ++it) {
        Block &block = **it;
...
void KCategorizedView::Private::rowsInserted(const QModelIndex &parent, int start, int end)
{
    ...
    for (int i = start; i <= end; ++i) {
        ...
        auto block = blockForIndex(index);
        ...
        Q_ASSERT(block && block->firstIndex.isValid());
    ...
    //BEGIN: update the items that are in quarantine in affected categories
    {
        ...
        auto block = blockForIndex(lastIndex);
        block->quarantineStart = block->firstIndex;
    }
    ...
    //BEGIN: mark as in quarantine those categories that are under the affected ones
    {
        ...
        const QModelIndex firstAffectedCategory = blockForIndex(firstIndex)->firstIndex;
        //BEGIN: order for marking as alternate those blocks that are alternate
        QList<QSharedPointer<Block>> blockList = blocks.values();
        qSort(blockList.begin(), blockList.end(), Block::lessThanPtr);
        QList<int> firstIndexesRows;
        foreach (const QSharedPointer<Block> block, blockList) {
            firstIndexesRows << block->firstIndex.row();
        }
        //END: order for marking as alternate those blocks that are alternate
        for (QHash<QString, QSharedPointer<Private::Block>>::Iterator it = blocks.begin(); it != blocks.end(); ++it) {
...
QSharedPointer<KCategorizedView::Private::Block> KCategorizedView::Private::blockForIndex(const QModelIndex& index) {
    const QModelIndex categoryIndex = index.model()->index(index.row(), proxyModel->sortColumn(), index.parent());

    const QString category(categoryIndex.data(KCategorizedSortFilterProxyModel::CategorySortRole).toString());
    for(QSharedPointer<Block> block : blocks.values(category)) {
        if(block->firstIndex.isValid() && block->firstIndex.row() <= categoryIndex.row() &&
                block->firstIndex.row() + block->items.count() >= categoryIndex.row()) {
...
{
    qDebug() << "_k_slotCollapseOrExpandClicked";
}
...
QRect KCategorizedView::visualRect(const QModelIndex &index) const
{
    ...
    auto block = d->blockForIndex(index);
    const int firstIndexRow = block->firstIndex.row();
    ...
    const QPoint blockPos = d->blockPosition(block);
...
{
    const auto block = d->blockForIndex(representative);
    QModelIndexList res;
    if (block->height == -1) {
        return res;
    }
...
void KCategorizedView::paintEvent(QPaintEvent *event)
{
    ...
    for(const auto block : d->blocks) {
        ...
        QPoint pos = d->blockPosition(block);
        pos.ry() -= height;
        option.rect.setTopLeft(pos);
    ...
    if (intersecting.first.isValid() && intersecting.second.isValid()) {
        ...
        QSharedPointer<Private::Block> block;
        while (i <= intersecting.second.row()) {
            ...
            if (i == indexToCheckIfBlockCollapsed) {
                ...
                block = d->blockForIndex(categoryIndex);
        ...
        if (option.rect.contains(mousePos)) {
            if (d->categoryDrawerV3 && d->hoveredBlock->height != -1 && *d->hoveredBlock != *block) {
                ...
                *d->hoveredBlock = *block;
                d->hoveredCategory = d->categoryForIndex(categoryIndex);
...
void KCategorizedView::mousePressEvent(QMouseEvent *event)
{
    ...
    QHash<QString, QSharedPointer<Private::Block>>::ConstIterator it(d->blocks.constBegin());
    ...
    switch (cursorAction) {
        ...
        case MoveDown: {
                if (d->hasGrid() || uniformItemSizes()) {
                    ...
                    const auto block = d->blockForIndex(current);
                    ...
                    const auto nextBlock = d->blockForIndex(nextIndex);
        ...
        case MoveUp: {
                if (d->hasGrid() || uniformItemSizes()) {
                    ...
                    const auto prevBlock = d->blockForIndex(prevIndex);
                                            ...
                                            int end)
{
    ...
    *d->hoveredBlock = Private::Block();
    ...
    //BEGIN: update the items that are in quarantine in affected categories
    {
        ...
        auto block = d->blockForIndex(lastIndex);
    ...
    {
        //BEGIN: order for marking as alternate those blocks that are alternate
        QList<QSharedPointer<Private::Block>> blockList = d->blocks.values();
        qSort(blockList.begin(), blockList.end(), Private::Block::lessThanPtr);
        QList<int> firstIndexesRows;
...
void KCategorizedView::updateGeometries()
{
    ...
    if (d->hasGrid()) {
        ...
        if (uniformItemSizes()) {
            ...
            QSize itemSize = sizeHintForIndex(lastIndex);
            itemSize.setHeight(d->highestElementInLastRow(*d->blockForIndex(lastIndex)) + spacing());
            lastItemRect.setSize(itemSize);
        }
    }
    ...
    if (verticalScrollMode() == ScrollPerItem) {
        ...
        Q_ASSERT(lastItemRect.height() != 0);
