...
DynamicTreeModel::DynamicTreeModel(QObject* parent) : QAbstractItemModel(parent), nextId(1) {
}
...
ModelInsertCommand::ModelInsertCommand(DynamicTreeModel* model, QObject* parent) : ModelChangeCommand(model, parent) {
}
...
ModelMoveCommand::ModelMoveCommand(DynamicTreeModel* model, QObject* parent) : ModelChangeCommand(model, parent) {
}
...
void ModelMoveCommand::doCommand() {
    ...
    for(int column = 0; column < m_numCols; ++column) {
        QList<qint64> l =
            m_model->m_childItems.value(srcParent.internalId())[column].mid(m_startRow, m_endRow - m_startRow + 1);
...
ModelResetCommand::ModelResetCommand(DynamicTreeModel* model, QObject* parent) : ModelMoveCommand(model, parent) {
}
...
void ModelChangeChildrenLayoutsCommand::doCommand() {
    ...
    Q_EMIT m_model->layoutAboutToBeChanged(parents);
        ...
        if(idx.parent() == parent1) {
            if(idx.row() == rowSize1 - 1) {
                ...
                m_model->changePersistentIndex(
                    idx, m_model->createIndex(idx.row() + 1, idx.column(), idx.internalPointer()));
            }
        } else if(idx.parent() == parent2) {
            if(idx.row() == 0) {
                m_model->changePersistentIndex(idx,
                                               m_model->createIndex(rowSize2 - 1, idx.column(), idx.internalPointer()));
            } else {
                m_model->changePersistentIndex(
                    idx, m_model->createIndex(idx.row() - 1, idx.column(), idx.internalPointer()));
            }
    ...
    Q_EMIT m_model->layoutChanged(parents);
}
