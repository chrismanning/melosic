...
CategoryProxyModel::CategoryProxyModel(QObject* parent) : QIdentityProxyModel(parent), pimpl(new impl(*this)) {
...
void CategoryProxyModel::onRowsMoved(const QModelIndex& sp, int sourceStart, int sourceEnd, const QModelIndex& dp,
                                     int destinationRow) {
    if(!pimpl->m_category)
        return;
...
void CategoryProxyModel::onRowsAboutToBeMoved(const QModelIndex& sp, int sourceStart, int sourceEnd,
                                              const QModelIndex& dp, int destinationRow) {
    if(!pimpl->m_category)
        return;
...
void CategoryProxyModel::onDataChanged(const QModelIndex& topleft, const QModelIndex& bottomright,
...
void CategoryProxyModel::impl::checkForConsistency() {
    ...
    for(auto i = 0; i < parent.rowCount(); ++i) {
        ...
        if(category == prev_category) {
            ...
            if(prev_block)
                BOOST_ASSERT_MSG(
                    prev_block->firstRow() + prev_block->count() == i,
                    (boost::format("Category Model Inconsistent: prev_block should end before current index.\n"
    ...
    if(idx > 0) {
        ...
        if(prev_category == category) {                               // same category => same block
            ...
            if(block) {                                         // already has own block
                ...
                const auto last = std::find_if(first, block_index.end(), [block](auto&& a) { return a != block; });
...
void CategoryProxyModel::impl::onRowsMoved(const QModelIndex&, int sourceStart, int sourceEnd, const QModelIndex&,
                                           int destinationRow) {
...
void CategoryProxyModel::impl::onDataChanged(const QModelIndex& istart, const QModelIndex& iend, const QVector<int>&) {
...
void CategoryProxyModelAttached::internal_update() {
    if(!m_model)
        return;
    assert(m_model);
    modelConns.push_back(
        connect(m_model, &CategoryProxyModel::rowsAboutToBeRemoved, [this](const QModelIndex&, int start, int end) {
    ...
    modelConns.push_back(connect(this, &CategoryProxyModelAttached::blockChanged, [this](auto&& block) {
        if(block)
            modelConns.push_back(connect(block, &Block::firstIndexChanged,
                                         [this](auto&& index) { Q_EMIT drawCategoryChanged(drawCategory()); }));
